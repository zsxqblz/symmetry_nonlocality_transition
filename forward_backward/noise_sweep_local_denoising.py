"""
Noise sweep experiment: generate clean DiT images, inject noise at multiple
timesteps, and denoise with local attention (conditional + unconditional).
Reports per-pixel MSE against the clean reference and classifier error.
"""
import argparse
import json
import os
...
OUTPUT_DIR = str(MODULE_DIR / "outputs" / "noise_sweep_local")
...
def run_noise_sweep(args):
    ...
    global_pipe = build_pipeline(args.model_id)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Noise sweep local denoising experiment")
